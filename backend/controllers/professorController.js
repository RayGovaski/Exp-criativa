...
        db.query("UPDATE Professor SET senha = ? WHERE id = ?", [hashedPassword, userId], (err) => {
...
        if (err) return res.status(500).json({ error: "Erro no banco de dados ao verificar turma." });
        if (result.length === 0) {
...
                a.id AS alunoId,
                a.nome AS alunoNome,
                COALESCE(cp.presenca, TRUE) AS presente
            FROM Aluno_Turma at
...
            if (err) return res.status(500).json({ error: "Erro ao buscar dados da chamada." });
