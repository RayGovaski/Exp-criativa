...
import CriarPerfilProfessor from '../comp-perfil-apoiador/CriarPerfilProfessor/CriarPerfilProfessor'; 
jest.mock('../comp-perfil-apoiador/CriarPerfilProfessor/CriarPerfilProfessor.css', () => ({})); 
describe('CriarPerfilProfessor', () => {
  ...
  /*
  test('não deve submeter o formulário se campos obrigatórios (nome, email, cpf) estiverem vazios', async () => {
    render(<CriarPerfilProfessor />);
    const submitButton = screen.getByRole('button', { name: /Criar Professor/i });
    await act(async () => {
      fireEvent.click(submitButton);
    });
    expect(consoleSpy).not.toHaveBeenCalledWith('Formulário de Criar Perfil de Professor submetido!');
  });
  */
});